...
namespace VoxelWorld.WorldGeneration.World
{
    public class WorldController : GenericMonoSingleton<WorldController>
    {
        ...
        private void LateUpdate()
        {
            ...
            float max = FIXED_WORLD_SIZE * ChunkService.chunkSize;
        ...
        public void GenerateFixedWorld()
        {
            for (int x = 0; x < FIXED_WORLD_SIZE; x++)
            {
                for (int z = 0; z < FIXED_WORLD_SIZE; z++)
                {
                    ...
                    {
                        kv.Value.RequiresCollider = true;
                        kv.Value.BuildMesh();
                    }
                }
            }
        ...
        public void SetupFog()
        {
            ...
            if (useLinearFog)
            {
                ...
                RenderSettings.fogStartDistance = (FIXED_WORLD_SIZE - 1) * 16;
                RenderSettings.fogEndDistance = FIXED_WORLD_SIZE * 16 * 1.2f;
            }
            else
            {
